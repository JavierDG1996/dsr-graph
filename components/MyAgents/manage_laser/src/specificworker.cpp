...
void SpecificWorker::compute()
{
    ...
    if(auto persons = G->get_nodes_by_type(person_type_name); not persons.empty())
        {
        //Obtener datos del laser y de las gausianas de las personas
        auto laser_node= G->get_node(laser_name).value();
        const auto &angles = G->get_attrib_by_name<laser_angles_att>(laser_node).value().get();
        const auto &dist = G->get_attrib_by_name<laser_dists_att>(laser_node).value().get();
        tuple laser=std::make_tuple(angles,dist);
        //for
        //Funcion modifique laser
        //Modificar laser
        //Actualizar nodo
        }
